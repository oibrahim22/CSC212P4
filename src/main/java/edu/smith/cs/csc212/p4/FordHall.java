...
private Map<String, Place> places = new HashMap<>();
	...
	public FordHall() {
		...
		basement.addExit(new Exit ("cageroom", "There are more stairs that lead down."));
		
		Place cageroom = insert(Place.create("cageroom", "You see a cage and some remaining bones. you look around for another exits, but there is not."));
		cageroom.addExit(new Exit("basement", "There are stairs that lead up."));
		...
		attic.addExit(new Exit("bedroom", "There is a big brown door to your left."));
		...
		Place bedroom = insert(Place.create("bedroom","There is a prefectly made bed. The room is also clean and dustless. You hear a movement"));
		bedroom.addExit(new Exit("attic", "There is no way out of this room. Head back to the attic!"));
		...
		secretRoom.addExit(new Exit("basement", "There is an exit to the basement."));
		int hallwayDepth = 5;
		int lastHallwayPart = hallwayDepth - 1;
		for (int i=0; i<hallwayDepth; i++) {
			Place hallwayPart = insert(Place.create("hallway"+i, "This is a very long hallway,its walls are marked red with the number "+(i+1)));
			if (i == 0) {
